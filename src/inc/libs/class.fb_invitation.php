...
class fb_invitation{
	...
	public function addFBInvite($invitor, $invitee, $periodIndex=0){
	...
	public function getInviteByInvitor($invitor, $periodIndex=0){
		$conn = ncAppUtil::getConnection();
		$query = "SELECT * FROM `".$this->invitetable."` WHERE invitor = ? AND periodIndex = ? ";
	...
	private function _prepareDB(){
		if(defined("DB_FBINVITE")){
			$this->invitetable = DB_FBINVITE;
		}else{
			//generate table if DB_PREFIX is exist
			if(defined("DB_PREFIX")){
				$this->invitetable = DB_PREFIX."__fbinvitations";
			}else{
	...
	private function _tablegenerate(){
		...
		$query = "CREATE TABLE IF NOT EXISTS `".$this->invitetable."` (
			  ...
			  `invitee` varchar(50) COLLATE utf8_bin NOT NULL,
