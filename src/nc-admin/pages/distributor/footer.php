...
<script>
	...
	function deleteItem(data){
		if(confirm("Do you really want to delete distributor - "+data.name+"( id:"+data.id +") ")){ //message, can customize
			$.ajax({
				type:"POST",
				url:"<?=$jsonPostURL?>",
	...
	function updateform(type, data){
		if(type=="add"){
			$form.find("h1 span").text("Add New <?=$objname?>");
			$form.find("h1 button").addClass("hide");

			$form.find("#distributor_id").val('');
			$form.find("#name").val('');
			$form.find("#contact_no").val('');
			$form.find("#email").val('');
			$form.find("#country").val('');
			$form.find("#exclusion").val(0);
			...
			$form.find("h1 span").text("Edit <?=$objname?>");
			$form.find("h1 button").removeClass("hide");

      		$form.find("#distributor_id").val(data.distributor_id);
			$form.find("#name").val(data.name);
			$form.find("#contact_no").val(data.contact_no);
			$form.find("#email").val(data.email);
			$form.find("#country").val(data.countryid);
			$form.find("#exclusion").val(data.exclusion_flag);
	...
	function createTable(data){
		...
		$.each(columndata, function(index,value){
			if(value.field == "no_follow_up_per"){
				columndata[index].formatter = function(value,row, index){
					return parseInt(value*10000,10) / 100 + "%";
				}
			}
		});
