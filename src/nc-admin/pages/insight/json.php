...
$isMobile = bcw_useragents::isMobile() ? 1 : 0;
...
switch($_REQUEST["method"]){
	...
	case "getUserTable":
		...
		LEFT JOIN ( SELECT uid, SUM(total_success) as `total`, SUM(success_fbshares) as `viashare`, SUM(success_fbinvitations) as `viaapprequest`, SUM(success_email) as `viaemail` FROM `'.$table_social.'` GROUP BY uid ) c ON c.uid = d.uid';
