...
<script>
	...
	function deleteItem(data){
		if(confirm("Do you really want to delete country - "+data.name+"( id:"+data.id +") ")){ //message, can customize
			$.ajax({
				type:"POST",
				url:"<?=$jsonPostURL?>",
	...
	function updateform(type, data){
		if(type=="add"){
			...
			$form.find("#name").val('');
			$form.find("#code").val('');
			$form.find("#default_language").val('');
			$form.find("#list_id").val('');
			...
			$form.find("#default_language").val(data.default_language);
			$form.find("#list_id").val(data.list_id);
