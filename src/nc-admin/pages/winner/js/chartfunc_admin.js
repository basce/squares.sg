...
function sortByAmount(a,b){
	return parseInt(a.amount,10) > parseInt(b.amount,10) ? -1 : parseInt(a.amount,10) < parseInt(b.amount,10) ? 1 : 0;
}
function f2(data, otherlabelname, maxitems, usePercentage){
		...
		$.each(data, function(index,value){
			if(index+1 < maxitems){
				...
				othersAmount += parseInt(value.amount,10);
			}
		});
		if(otherlabel){
			if(usePercentage){
...
function getArrayItemForChart(ar, index){
	if(index < ar.length){
		return ar[index];
	}else{
		return {
...
function createColumnType1(data, element){
	...
	var seriesgourp = [{
			name: 'percentage',
			data: [
				getArrayItemForChart(data1a.data, [1]),
				getArrayItemForChart(data1b.data, [1]),
				getArrayItemForChart(data1c.data, [1])]
		}, {
			name: '',
			data: [
				getArrayItemForChart(data1a.data, [0]),
				getArrayItemForChart(data1b.data, [0]),
				getArrayItemForChart(data1c.data, [0])]
		}];
