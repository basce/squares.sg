...
<script>
	...
	function sendEmail(data){
		if(confirm("Do you really want to send a reset password email to admin - "+data.username+"( id:"+data.aid +") ")){ //message, can customize
			$.ajax({
				type:"POST",
				url:"<?=$jsonPostURL?>",
				data:{method:"retrievepw",nctable:"<?=$ncTable?>", id:data.aid},
				dataType:'json',
				timeout:5000,
				success:function(d){
					displayMessage(d);
	...
	function deleteItem(data){
		if(confirm("Do you really want to delete admin - "+data.username+"( id:"+data.aid +") ")){ //message, can customize
			$.ajax({
				type:"POST",
				url:"<?=$jsonPostURL?>",
	...
	function updateform(type, data){ // form fields customize
		if(type=="add"){
			...
			$form.find('button[name="submit"]').val('add').text('Done');
			...
			$form.find("#location").val(parseInt(data.locationid,10)?data.locationid:0);
			$form.find('input[name="aid"]').val(data.aid);
			$form.find('button[name="submit"]').val('edit').text("Done");
	...
	function createActionButton(ar){
		...
		$.each(ar, function(index, value){
			switch(value){
				...
				break;
				case "retrievepw":
					a += '<a class="retrievepw" href="javascript:void(0)" title="Send Reset Password Email"><i class="glyphicon glyphicon-envelope"></i></a>';
				break;
			}
		});
		return a;
			...
			*/
			...
			columndata.push( {
                        ...
                        events: {
							...
							'click .retrievepw':function(e, value, row, index){
								sendEmail(row);
							}
						},
    ...
    function setForm(type){
    	switch(type){
    		case 1:
    			...
    			$("#passwordHelp").text("Password for Regional Admin can be set here, and email is optional.");
    		break;
    		default:
    			$("#email").attr({required:true});
    			$("#password").removeAttr("required");
    			$("#passwordHelp").text("Password is optional, you can send password reset email to user after account creation via the action button below.");
    		break;
    	}
    }
	...
	$(function(){
		...
		$("#level").change(function(e){
			setForm(parseInt($(this).val(),10));
		});
<?php if($user["level"] == 4 || $user["level"] == 2){ ?>
		setForm(0);
<?php }else{ ?>
		setForm(1);
<?php } ?>
